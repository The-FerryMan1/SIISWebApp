  ...
  <UPage class="p-5">
    <UContainer class="flex flex-col items-center my-10">
      ...
      <div class="flex flex-col text-center justify-center items-center p-3 my-3 gap-2">
        <img src="../../assets/img/brand.png" class="w-full h-24 min-w-24 min-h-24 block object-contain"
          alt="Brand Logo" />
        ...
        <UPageCard class="w-full" variant="outline">
          <div class="flex flex-col items-center gap-4 py-16">
            ...
            <p class="text-muted text-center max-w-md">
              Your internship application has been successfully submitted. We will review your application and get back
              to you via email.
            </p>
            <UButton color="primary" variant="outline" icon="i-lucide-rotate-ccw" label="Submit Another Application"
      ...
      <!-- Form -->
      <template v-else>
        <UForm ref="form" @submit="onReview" :schema="OnBoardUpdateDtoSchema" :state="state" class="space-y-6 w-full"
          ...
          <UPageCard title="Student Information" icon="i-lucide-user" variant="outline">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <UFormField name="student.dateOfBirth" label="Date of Birth" required>
                <UInput :max="maxDate.toISOString().split('T')[0]" v-model="state.student.dateOfBirth" type="date"
                  class="w-full" />
              </UFormField>
              <UFormField name="student.gender" label="Gender" required>
                <USelect v-model="state.student.gender" placeholder="Select gender" :items="genderItems"
                  class="w-full" />
              </UFormField>
              <UFormField name="student.gradeLevel" label="Grade Level" required>
                <USelect v-model="state.student.gradeLevel" placeholder="Select grade level" :items="gradeLevelItems"
                  class="w-full" />
              </UFormField>
            ...
            <UFormField name="student.address" label="Address" required class="mt-4">
              <UTextarea v-model="state.student.address" placeholder="Enter your complete address" class="w-full"
                :rows="3" />
            </UFormField>
          </UPageCard>
          ...
          <UPageCard title="School Details" icon="i-lucide-building" variant="outline">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <UFormField name="school.email" label="Contact Person's Email" required>
                <UInput v-model="state.school.email" type="email" placeholder="Enter the email of the contact person"
                  class="w-full" />
              </UFormField>
              <UFormField name="school.contactNumber" label="Contact Person's Number" required>
                <UInput v-model="state.school.contactNumber" placeholder="Enter the contact person's phone number"
                  class="w-full" />
              </UFormField>
            </div>
            <UFormField name="school.address" label="School Address" required class="mt-4">
              <UTextarea v-model="state.school.address" placeholder="Enter the school address" :rows="3"
                class="w-full" />
            </UFormField>
          </UPageCard>
          ...
          <UPageCard title="Internship Details" icon="i-lucide-file" variant="outline">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <UFormField name="internship.internshipNature" label="Nature of Internship" required>
                <USelect v-model.number="state.internship.internshipNature" placeholder="Select internship nature"
                  :items="internshipNatureItems" class="w-full" />
              </UFormField>
              <UFormField name="internship.strand" v-if="isSeniorHigh" label="Strand" required>
                <USelect v-model.number="state.internship.strand" placeholder="Select strand" :items="strandItems"
                  class="w-full" />
              </UFormField>
              <UFormField v-if="isCollege" name="internship.degree" label="Degree" required>
                <USelect v-model="state.internship.degree" placeholder="Select degree" :items="degreeItems"
                  class="w-full" />
              </UFormField>
              <UFormField name="internship.startDate" label="Start Date" required>
                <UInput :min="minStartDate.toISOString().split('T')[0]" v-model="state.internship.startDate" type="date"
                  class="w-full" />
              </UFormField>
              <UFormField name="internship.internshipTotalHours" label="Total Internship Hours" required>
                <UInput v-model="state.internship.internshipTotalHours" type="number"
                  placeholder="Enter the total hours of internship" min="0" class="w-full" />
              </UFormField>
      ...
      <UModal v-model:open="isOpen" title="Review Application" size="xl">
        <template #body>
          <div class="flex flex-col gap-5" v-if="state.student">
            <UPageCard title="Student Information" icon="i-lucide-user">
              <ul class="space-y-1">
                <li v-for="(value, key) in state.student" :key="key"
                  ...
                  <span class="font-bold">
                    ...
                    <template v-else-if="key === 'gradeLevel' && typeof value === 'number'">{{
                      gradeLevelFinder(value) }}</template>
            ...
            <UPageCard title="School Details" icon="i-lucide-building">
              <ul class="space-y-1">
                <li v-for="(value, key) in state.school" :key="key"
                  class="flex items-center justify-between py-1 border-b border-default last:border-0">
                  <span class="text-muted capitalize">{{ key }}:</span>
            ...
            <UPageCard title="Internship Details" icon="i-lucide-file">
              <ul class="space-y-1">
                <li v-for="(value, key) in state.internship" :key="key"
                  class="flex items-center justify-between py-1 border-b border-default last:border-0">
                  <span class="text-muted capitalize">{{ key }}:</span>
                  <span class="font-bold">
                    <template v-if="key === 'internshipNature' && typeof value === 'number'">{{
                      internshipNatureFinder(value) }}</template>
                    <template v-else-if="key === 'degree' && typeof value === 'number'">{{ degreeFinder(value)
                    }}</template>
                    <template v-else-if="key === 'strand' && typeof value === 'number'">{{ strandFinder(value)
                    }}</template>
            ...
            <UPageCard title="Uploaded Requirements" icon="i-lucide-folder">
              <ul class="space-y-1">
                <li v-for="(file, index) in state.requirements" :key="index"
                  class="flex items-center justify-between py-1 border-b border-default last:border-0">
        ...
        <template #footer>
          <div class="w-full flex items-center justify-between">
            <UButton variant="ghost" color="neutral" icon="i-lucide-arrow-left" @click="isOpen = false">
              Go Back & Edit
            </UButton>
            <UButton color="primary" icon="i-lucide-send" label="Confirm & Submit" :loading="isSubmitting"
              @click="onConfirm" />
          </div>
        </template>
      </UModal>
