...
const profileSubmit = async (event: FormSubmitEvent<ProfileSchema>) => {
    ...
    const confirm = await confirmationModal.open({title: "Update user information", description: "Are you sure?"})
...
<template>
    <UMain class="space-y-5">
        ...
        <UCard title="Edit Profile">
            <UForm :schema="profileSchema" :state="profileState" v-if="user" class="space-y-4" @submit="profileSubmit"
                loading-auto>
                ...
                <UFormField name="email" label="Email" required>
                    <UInput v-model="profileState.email" placeholder="Enter new email" class="w-full" disabled />
                </UFormField>
        ...
        <UCard title="Change Password">
            <UForm ref="changePassForm" :state="changePassState" :schema="changePassSchema" @submit="changePassSubmit"
                ...
                <UFormField name="confirm" label="Confirm Password" required>
                    <UInput type="password" v-model="changePassState.confirm" placeholder="Confirm new password"
                        class="w-full" />
                </UFormField>
                <div class="w-full flex justify-end items-center my-8">
